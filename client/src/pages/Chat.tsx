...
type BranchRef = { _id: string; name: string; branchCode: string };
...
type Conversation = {
  ...
  unreadAdmin: number;
  // Present for superadmins: branches this customer has ordered from
  customerBranches?: BranchRef[];
};
...
export default function Chat() {
  ...
  useEffect(() => {
    ...
    // Conversation list update (new message, unread reset, etc.)
    socket.on("conversation_updated", (updated: Conversation) => {
      setConversations((prev) =>
        prev
          .map((c) =>
            // Preserve the badge annotation across live updates
            c._id === updated._id
              ? { ...updated, customerBranches: c.customerBranches }
              : c,
          )
          .sort((a, b) =>
  ...
  const handleSend = () => {
    ...
    socketRef.current.emit(
      ...
      (ack: { success: boolean; data?: Message; message?: string }) => {
        setSending(false);
        if (!ack?.success) {
          toastError(ack?.message ?? "Failed to send message.");
          setText(trimmed);
        } else {
  ...
  const filtered = conversations.filter((c) => {
    ...
    const branchNames = (c.customerBranches ?? [])
      .map((b) => b.name.toLowerCase())
      .join(" ");
    const q = convSearch.toLowerCase();
    return name.includes(q) || email.includes(q) || branchNames.includes(q);
  });
  ...
  for (const msg of messages) {
    const label = new Date(msg.createdAt).toDateString();
  ...
  return (
    <div className="flex h-[calc(100vh-2rem)] gap-4">
      ...
      ══════════════════════════════════════════ */}
      <div className={`w-96 shrink-0 flex flex-col rounded-2xl border ${surface} ${border} overflow-hidden`}>
        {/* Header */}
        <div className={`px-4 py-4 border-b ${border}`}>
          <div className="flex items-center gap-2 mb-3">
            ...
            {totalUnread > 0 && (
              <span className="text-xs font-bold bg-[#DA291C] text-white px-2 py-0.5 rounded-full">
          ...
          <div className={`flex items-center gap-2 h-9 px-3 rounded-xl border ${isDark ? "bg-[#121212] border-[#2E2E2E] text-white" : "bg-[#F8F5F2] border-[#E5E2DE] text-[#232323]"}`}>
        ...
        <div className="flex-1 overflow-y-auto">
          {loadingConvs ? (
            ...
            filtered.map((conv) => {
              ...
              const branches = conv.customerBranches ?? [];
              return (
                <button
                  key={conv._id}
                  onClick={() => openConversation(conv)}
                  className={`w-full flex items-start gap-3 px-4 py-3 border-b ${border} text-left transition-colors cursor-pointer
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className={`text-sm font-semibold truncate ${headline}`}>
                        {conv.customer.firstname} {conv.customer.lastname}
                      </p>
                      {conv.lastMessageAt && (
                        <p className={`text-[10px] shrink-0 ${muted}`}>
                    ...
                    {/* Branch badges — superadmin view */}
                    {branches.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1.5">
                        {branches.map((b) => (
                          <span
                            key={b._id}
                            className="inline-flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded-md bg-[#E8F5EF] dark:bg-[#0A3D3D] text-[#007A53] dark:text-[#4CAF50]"
                            title={b.name}
                          >
                            <Store size={9} />
                            {b.name}
        ...
        {activeConv ? (
          <>
            ...
            <div className={`flex items-center gap-3 px-5 py-3.5 border-b ${border}`}>
              ...
              {/* Branch badges in the thread header too */}
              {(activeConv.customerBranches ?? []).length > 0 && (
                <div className="flex flex-wrap gap-1 shrink-0">
                  {(activeConv.customerBranches ?? []).map((b) => (
                    <span
                      key={b._id}
                      className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-md bg-[#E8F5EF] dark:bg-[#0A3D3D] text-[#007A53] dark:text-[#4CAF50]"
                    >
                      <Store size={9} />
                      {b.name}
                    </span>
            ...
            <div className={`px-4 py-3 border-t ${border} flex items-end gap-3`}>
              <textarea
                ...
                rows={1}
                className={`flex-1 resize-none rounded-xl border px-4 py-2.5 text-sm outline-none max-h-28 ${isDark ? "bg-[#121212] border-[#2E2E2E] text-white placeholder-[#555]" : "bg-[#F8F5F2] border-[#E5E2DE] text-[#232323] placeholder-[#999]"}`}
