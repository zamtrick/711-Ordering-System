...
import { ChevronLeft, Send, MessageCircle, WifiOff, User } from "lucide-react-native";
...
// --------------------------------------------------
// Per-order delivery chat — rider <-> customer.
// Opened from a delivery card's "Message customer" button.
// --------------------------------------------------
...
export default function DeliveryChat() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const convRes = await api.get(`/chat/order/${oid}`);
  ...
  useEffect(() => {
    ...
    const handler = (msg: ChatMessage) => {
      if (msg.conversation && convIdRef.current && msg.conversation !== convIdRef.current) return;
      setMessages((prev) => {
  ...
  const customerName = convo?.customer
    ? `${convo.customer.firstname ?? ""} ${convo.customer.lastname ?? ""}`.trim() || "Customer"
    : "Customer";
        ...
        {!isMe && (
          <View style={[styles.avatar, { backgroundColor: "#007A53" }]}>
            <Text style={styles.avatarText}>
              {(msg.sender?.firstname?.[0] ?? "C").toUpperCase()}
            </Text>
          </View>
        )}
        <View
        ...
        <View style={styles.headerInfo}>
          ...
          <View style={styles.headerStatus}>
            ...
            <Text style={[styles.statusText, { color: colors.muted }]}>
              {typeof oid === "string" ? `Order #${oid.slice(-6).toUpperCase()}` : "Delivery chat"}
      ...
      {loading ? (
        ...
        <View style={styles.centered}>
          ...
          <Pressable onPress={() => router.back()} style={styles.backBtn2}>
            <Text style={styles.backBtn2Text}>Back to deliveries</Text>
          </Pressable>
        </View>
      ) : (
        <KeyboardAvoidingView
          ...
          {messages.length === 0 ? (
            <View style={styles.centered}>
              ...
              <Text style={[styles.emptyText, { color: colors.muted }]}>
                Message {customerName} — ETA, landmarks, gate codes.
              </Text>
            </View>
          ) : (
            <FlatList
...
const styles = StyleSheet.create({
  ...
  backBtn2: { marginTop: 18, height: 44, paddingHorizontal: 24, borderRadius: 12, backgroundColor: "#007A53", alignItems: "center", justifyContent: "center" },
